...
if __name__ == '__main__':
    ...
    with tf.name_scope('phrase_embedding'):
        embed_aggre_sememe_l = tf.reduce_sum(embed_sememe_l, axis=0, keepdims=True, name="embed_word_l")
        embed_aggre_sememe_r = tf.reduce_sum(embed_sememe_r, axis=0, keepdims=True, name="embed_word_r")
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(total_epoch):
            ...
            for current_num, train_tup in enumerate(hownet.comp_train):
                ...
                _, summary_i, step_num, loss_i = sess.run(
                    [train_one_example, merged, global_step, cross_entropy_mean],
